...
function bb_theme_comment_form() {
  ...
  $req_text = '<span class="required">（MUST）</span>';
  ...
  // $comment_field 設定
  $comment_field = '<p class="comment-form-comment"><label for="comment">' . 'comments'
    . $req_text . '</label>'
  ...
  $args = array(
    ...
    'logged_in_as'         => sprintf(
      ...
      sprintf(
        __('Logging as <a href="%1$s">%2$s</a> <br class="is-mobile">（<a href="%3$s">Log out</a>）'),
        get_edit_user_link(),
